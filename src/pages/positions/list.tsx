...
import { getPositions, createPosition, updatePosition, deletePosition } from "../../api/user";
...
export function PositionsList() {
  ...
  const createMutation = useMutation({
    mutationFn: (positionData: PositionFormData) => createPosition({ positionData }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ positionId, positionData }: { positionId: number; positionData: Partial<PositionFormData> }) =>
  ...
  const handleSave = () => {
    if (!formData) return;
    if (editingPosition) {
      updateMutation.mutate({ positionId: editingPosition.id, positionData: formData });
    } else {
  ...
  return (
    <div style={{ padding: "20px", marginTop: "var(--header-height)" }}>
      ...
      <table style={{
        ...
        <thead>
          <tr style={{ backgroundColor: "#686ACF", color: "white" }}>
            ...
            <th style={{ padding: "12px", textAlign: "center" }}>Регистрация</th>
            <th style={{ padding: "12px", textAlign: "center" }}>Категории</th>
            <th style={{ padding: "12px", textAlign: "center" }}>Карточки</th>
            <th style={{ padding: "12px", textAlign: "center" }}>Пользователи</th>
            <th style={{ padding: "12px", textAlign: "center" }}>Должности</th>
            {canManagePositions && <th style={{ padding: "12px", textAlign: "center" }}>Действия</th>}
          </tr>
        </thead>
        <tbody>
          {positions?.map((position: Position) => (
            ...
            >
              ...
              <td style={{ padding: "12px", textAlign: "center" }}>{position.hierarchy_level}</td>
      ...
      {(isCreateOpen || editingPosition) && formData && (
        <ModalWrapper setIsOpen={() => { setEditingPosition(null); setIsCreateOpen(false); setFormData(null); }}>
          <div style={{
            display: "flex",
            ...
            <h3>{editingPosition ? "Редактирование должности" : "Создание должности"}</h3>
            
            <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
              <label style={{ fontSize: "14px", fontWeight: 600 }}>Название должности</label>
              <input
                type="text"
                value={formData.name}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
              <label style={{ fontSize: "14px", fontWeight: 600 }}>Уровень иерархии (1-10, 1 = высший)</label>
              <input
                type="number"
                min="1"
                max="10"
            ...
            <div style={{
              ...
              <label style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "14px" }}>
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, can_edit_categories: e.target.checked })}
                />
              ...
              <label style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "14px" }}>
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, can_delete_categories: e.target.checked })}
                />
                Удаление категорий
              </label>
              <label style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "14px" }}>
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, can_edit_cards: e.target.checked })}
                />
              ...
              <label style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "14px" }}>
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, can_delete_cards: e.target.checked })}
                />
                Удаление карточек
              </label>
              <label style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "14px" }}>
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, can_edit_any_user: e.target.checked })}
                />
              ...
              <label style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "14px" }}>
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, can_manage_positions: e.target.checked })}
                />
            ...
            <div style={{
              ...
              <Button
                label="Отмена"
                fillColor
                onClick={() => { setEditingPosition(null); setIsCreateOpen(false); setFormData(null); }}
              />
              <Button
                label="Сохранить"
                fillColor
